...
def write_fairness_trial(output, data_dir, post_fix = ''):
    # writing json may fail because prob_values has np_arrays
    if 'prob_values' in output:
        for var in output['prob_values']:
            try:
                output['prob_values'][var] = output['prob_values'][var].tolist()
            except:
                pass
